            ...
            if (value != null) {
                if (viewFormat != null) {
                    if (this == UPLOAD || this == DOWNLOAD) {
                        if (value >= (100 * roundingValue)) {
                            return viewFormat.format(value / roundingValue);
                        } else {
                            return new DecimalFormat("@@ Mbps").format(value / roundingValue);
                        }
                    } else {
                        return viewFormat.format(value / roundingValue);
                    }
    ...
    final class GroupViewArrayAdapter extends ArrayAdapter<GroupView> {
        ...
        View getView(int position, View rowView, @NonNull ViewGroup parent) {
            if (rowView == null) {
                ...
                viewHolder.title = rowView.findViewById(R.id.test_view_info_list_title);
                viewHolder.result = rowView.findViewById(R.id.test_view_info_list_result);
                viewHolder.image = rowView.findViewById(R.id.test_view_info_list_image);
                viewHolder.progressBar = rowView.findViewById(R.id.test_view_info_list_progress_bar);
