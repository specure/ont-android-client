...
public class CustomizableGraphView extends View implements GraphView {
    ...
    private List<GraphLabel> labelVMaxList = new ArrayList<GraphLabel>();
...
//        this is rect around graph grid
//    	gridCanvas.drawRect(startX, startY, endX, endY-30, p);
    	for (int cy = 0; cy <= gridRows; cy++) {
    		final int posY = Math.max(h - partH * cy, 0);
    		gridCanvas.drawLine(
    		        (cy == 0 || cy == gridRows ? 15 : coordFW(x - 5, relW)), // first and last row start point is different than the other
    				coordFH(posY, relH),
                    endX,
                    coordFH(posY, relH),
                    p);
...
//    	}
    	...
    	if (labelRowList != null && canvas != null) {
    		for (int i = 0; i < labelRowList.size(); i++) {
            	...
            	canvas.drawText(
            	        label.getText(),
                        coordFW((int) (gridX - 9), relW),
                        coordFH((int) ((gridY + h -5) - partH * i + 7), relH),
                        paintNames);
    		}
		}
