...
import at.specure.android.util.network.network.WifiNetworkInfo
                        ...
                        is CellSignalStrengthLte -> {
                            if (nrConnectionState == NRConnectionState.NSA) {
                                if ((cellInfo is CellInfoNr) && (cellInfo.cellSignalStrength is CellSignalStrengthNr)) {
                                    ...
                                    if (!signalStrengths.isNullOrEmpty()) {
                                        val cellSignalStrengthNr = signalStrengths[0]
                                        return SignalStrengthInfoNr(
                                            transport = TransportType.CELLULAR,
                                            value = cellSignalStrengthNr.ssRsrp.checkValueAvailable()?.let { nrSignal -> -abs(nrSignal) },
