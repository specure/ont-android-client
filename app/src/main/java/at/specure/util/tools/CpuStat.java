...
public abstract class CpuStat {
	...
	public static class CpuUsage {
		...
		protected List<CoreUsage> coreUsageList = new ArrayList<CoreUsage>();
			...
			synchronized (coreUsageList) {
				...
				try {
					for (int i = 0; i < coreUsageList.size(); i++) {
						...
						result[i] = (cpu2[i] - cpu1[i]) / ((cpu2[i] + idle2[i]) - (cpu1[i] + idle1[i]));
		...
		public static class CoreUsage {
			...
			public CoreUsage(int coreNumber, long user, long nice, long system, long idle, long iowait, long irq, long softirq) {
