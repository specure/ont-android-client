...
import at.specure.minidns.core.record.DelegatingDnssecRR;
...
public class DNSSECClient extends ReliableDNSClient {
     ...
     */
    public DNSSECClient(Context context) {
     ...
     */
    public DNSSECClient(Context context, DNSCache cache) {
        super(context, cache);
    ...
    private Set<UnverifiedReason> verifySignedRecords(Question q, RRSIG rrsig, List<Record<? extends Data>> records) throws IOException {
        ...
        if (rrsig.typeCovered == Record.TYPE.DNSKEY) {
        ...
        } else if (q.type == Record.TYPE.DS && rrsig.signerName.equals(q.name)) {
            ...
            DNSSECMessage dnskeyRes = queryDnssec(rrsig.signerName, Record.TYPE.DNSKEY);
            if (dnskeyRes == null) {
    ...
    private Set<UnverifiedReason> verifySecureEntryPoint(Question q, final Record<DNSKEY> sepRecord) throws IOException {
        ...
        DNSSECMessage dsResp = queryDnssec(sepRecord.name, Record.TYPE.DS);
        if (dsResp == null) {
        ...
        if (delegation == null && dlv != null && !dlv.isChildOf(sepRecord.name)) {
            DNSSECMessage dlvResp = queryDnssec(DNSName.from(sepRecord.name, dlv), Record.TYPE.DLV);
            if (dlvResp != null) {
