...
import at.specure.minidns.core.record.InternetAddressRR;
import at.specure.minidns.core.record.NS;
import at.specure.minidns.core.record.RRWithTarget;
...
public class IterativeDNSClient extends AbstractDNSClient {
     ...
     */
    public IterativeDNSClient(Context context) {
        super(context);
    }
     ...
     */
    public IterativeDNSClient(Context context, DNSCache cache) {
        super(context, cache);
    }
    ...
    private DNSMessage queryRecursive(ResolutionState resolutionState, DNSMessage q, InetAddress address, DNSName authoritativeZone) throws IOException {
        ...
        for (Record<? extends Data> record : authorities) {
            ...
            if (question.name.equals(name) && (question.type == Record.TYPE.A || question.type == Record.TYPE.AAAA))
                continue;
    ...
    private IpResultSet resolveIpRecursive(ResolutionState resolutionState, DNSName name) throws IOException {
        ...
        if (ipVersionSetting.v4) {
            ...
            Question question = new Question(name, Record.TYPE.A);
            ...
            if (aMessage != null) {
                for (Record<? extends Data> answer : aMessage.answerSection) {
                    ...
                    } else if (answer.type == Record.TYPE.CNAME && answer.name.equals(name)) {
        ...
        if (ipVersionSetting.v6) {
            ...
            Question question = new Question(name, Record.TYPE.AAAA);
            final DNSMessage query = getQueryFor(question);
            DNSMessage aMessage = queryRecursive(resolutionState, query);
            if (aMessage != null) {
                for (Record<? extends Data> answer : aMessage.answerSection) {
