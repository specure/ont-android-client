...
import at.specure.client.v2.task.DnsTask;
import at.specure.client.v2.task.HttpProxyTask;
import at.specure.client.v2.task.NonTransparentProxyTask;
...
import at.specure.client.v2.task.TcpTask;
import at.specure.client.v2.task.TracerouteTask;
import at.specure.client.v2.task.UdpTask;
import at.specure.client.v2.task.VoipTask;
import at.specure.client.v2.task.WebsiteTask;
...
public class QualityOfServiceTest implements Callable<QoSResultCollector> {
	...
	public QualityOfServiceTest(TestClient client, TestSettings nnTestSettings, boolean onlyVoipTest) {
	 ...
	 */
	protected QualityOfServiceTest(TestClient client, TestSettings nnTestSettings, int dummyParam) {
		this.client = client;
     ...
     * @param client
     */
    public QualityOfServiceTest(TestClient client, TestSettings nnTestSettings) {
    	System.out.println("\n\n---- Initializing QoS Tests ----\n");
    	this.onlyVoipTest = false;
    	this.client = client;
		...
		for (TaskDesc taskDesc : client.getTaskDescList()) {
			...
			if (onlyVoipTest) {
				...
				else if (TestClient.TASK_NON_TRANSPARENT_PROXY.equals(taskId)) {
				...
				else if (TestClient.TASK_UDP.equals(taskId)) {
				...
				else if (TestClient.TASK_WEBSITE.equals(taskId)) {
