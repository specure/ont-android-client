...
public class QoSServerResult implements Serializable {
	...
	public QoSServerResult(JsonObject json) {
		try {
            testType = null;
            if (json.has(JSON_KEY_TESTTYPE)) {
                testType = QoSTestResultEnum.valueOf(json.get(JSON_KEY_TESTTYPE).getAsString().toUpperCase(Locale.US));
            }

            testDescription = "";
            if (json.has(JSON_KEY_TEST_DESCRIPTION)) {
                testDescription = json.get(JSON_KEY_TEST_DESCRIPTION).getAsString();
            }

            testSummary = "";
            if (json.has(JSON_KEY_TEST_SUMMARY)) {
                testSummary = json.get(JSON_KEY_TEST_SUMMARY).getAsString();
            }

            successCount = 0;
            if (json.has(JSON_KEY_SUCCESS_COUNT)) {
                successCount = Integer.valueOf(json.get(JSON_KEY_SUCCESS_COUNT).getAsString());
            }

            failureCount = 0;
            if (json.has(JSON_KEY_FAILURE_COUNT)) {
                failureCount = Integer.valueOf(json.get(JSON_KEY_FAILURE_COUNT).getAsString());
            }

            onFailureBehaviour = DEFAULT_ON_FAILURE_BEHAVIOUR;
            if (json.has(JSON_KEY_ON_FAILURE_BEHAVIOUR)) {
                onFailureBehaviour = json.get(JSON_KEY_ON_FAILURE_BEHAVIOUR).getAsString();
            }

			uid = json.get(JSON_KEY_UID).getAsLong();
			JsonObject resultObj = json.getAsJsonObject(JSON_KEY_RESULT_MAP);
		
			if (resultObj != null) {
				Iterator<Map.Entry<String, JsonElement>> keys = resultObj.entrySet().iterator();
				while (keys.hasNext()) {
					Map.Entry<String, JsonElement> next = keys.next();
					String key = next.getKey();
					resultMap.put(key, next.getValue().getAsString());
				}
			}
		}
		catch (Throwable t) {
