...
public class TracerouteTask extends AbstractQoSTask {
	...
	public TracerouteTask(QualityOfServiceTest nnTest, TaskDesc taskDesc, int threadId) {
		...
		this.host = null;
		if (taskDesc.getParams().containsKey(PARAM_HOST)) {
			this.host = ((JsonElement) taskDesc.getParams().get(PARAM_HOST)).getAsString();
		}
		...
		value = null;
		if (taskDesc.getParams().containsKey(PARAM_MAX_HOPS)) {
			value = ((JsonElement) taskDesc.getParams().get(PARAM_MAX_HOPS)).getAsString();
		}
