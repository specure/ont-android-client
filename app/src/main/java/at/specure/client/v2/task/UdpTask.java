...
public class UdpTask extends AbstractQoSTask {
    ...
    public UdpTask(QualityOfServiceTest nnTest, TaskDesc taskDesc, int threadId) {
        ...
        String value = null;
        if (taskDesc.getParams().containsKey(PARAM_NUM_PACKETS_INCOMING)) {
            value = ((JsonElement) taskDesc.getParams().get(PARAM_NUM_PACKETS_INCOMING)).getAsString();
        }
        this.packetCountIncoming = value != null ? Integer.valueOf(value) : null;

        value = null;
        if (taskDesc.getParams().containsKey(PARAM_NUM_PACKETS_OUTGOING)) {
            value = ((JsonElement) taskDesc.getParams().get(PARAM_NUM_PACKETS_OUTGOING)).getAsString();
        }
        this.packetCountOutgoing = value != null ? Integer.valueOf(value) : null;

        value = null;
        if (taskDesc.getParams().containsKey(PARAM_PORT)) {
            value = ((JsonElement) taskDesc.getParams().get(PARAM_PORT)).getAsString();
        }
        this.incomingPort = value != null ? Integer.valueOf(value) : null;

        value = null;
        if (taskDesc.getParams().containsKey(PARAM_PORT_OUT)) {
        ...
        value = null;
        if (taskDesc.getParams().containsKey(PARAM_TIMEOUT)) {
        ...
        try {
            ...
            try {
                            ...
                            try {
                                if (request.startsWith("GET UDPPORT")) {
                                    ...
                                    if (response != null && response.startsWith("OK")) {
                                        ...
                                        Future<UdpPacketData> udpOutTimeoutTask = TestClient.getCommonThreadPool().submit(new Callable<UdpPacketData>() {
                ...
                if (this.packetCountIncoming != null && this.incomingPort != null) {
                    ...
                    Future<UdpPacketData> udpInTimeoutTask = TestClient.getCommonThreadPool().submit(new Callable<UdpPacketData>() {
