...
public class NonTransparentProxyTask extends AbstractQoSTask {
	...
	public NonTransparentProxyTask(QualityOfServiceTest nnTest, TaskDesc taskDesc, int threadId) {
		...
		String value = null;
		if (taskDesc.getParams().containsKey(PARAM_PROXY_REQUEST)) {
			value = ((JsonElement) taskDesc.getParams().get(PARAM_PROXY_REQUEST)).getAsString();
		}
		String requestString = value;
		...
		this.port = 0;
		if (taskDesc.getParams().containsKey(PARAM_PROXY_PORT)) {
			this.port = ((JsonElement) taskDesc.getParams().get(PARAM_PROXY_PORT)).getAsInt();
		}

		value = null;
		if (taskDesc.getParams().containsKey(PARAM_PROXY_TIMEOUT)) {
			value = ((JsonElement) taskDesc.getParams().get(PARAM_PROXY_TIMEOUT)).getAsString();
		}
