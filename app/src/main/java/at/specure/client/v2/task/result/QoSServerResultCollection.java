...
public class QoSServerResultCollection implements Serializable {
	...
	public static class QoSResultStats {
		...
		private HashMap<QoSTestResultEnum, QoSResultStats> resultCounter = 
				new HashMap<QoSTestResultEnum, QoSResultStats>();
	...
	public QoSServerResultCollection(JsonArray json) throws JsonParseException {
		this.testResultArray = json;

		try {
			this.testResultDetails = json.get(0).getAsJsonObject().getAsJsonArray(JSON_KEY_DETAIL);
	        this.testResultDesc = json.get(0).getAsJsonObject().getAsJsonArray(JSON_KEY_DESC);
	        this.testResultTestDesc = json.get(0).getAsJsonObject().getAsJsonArray(JSON_KEY_TEST_DESC);
		}
		catch (Exception e) {
			//e.printStackTrace();
        ...
        //read test results:
        for (int i = 0; i < testResultDetails.size(); i++) {
        	QoSServerResult result = new QoSServerResult(testResultDetails.get(i).getAsJsonObject());
        ...
        try {
            for (int i = 0; i < testResultTestDesc.size(); i++) {
            	try {
	            	JsonObject desc = testResultTestDesc.get(i).getAsJsonObject();
                    String testType = "";
                    if (desc.has("test_type")) {
                        testType = desc.get("test_type").getAsString().toUpperCase(Locale.US);
                    }
	            	QoSTestResultEnum type = QoSTestResultEnum.valueOf(testType);
                    String testDesc = "";
                    if (desc.has("desc")) {
                        testDesc = desc.get("desc").getAsString();
                    }
                    String testName = "";
                    if (desc.has("name")) {
                        testName = desc.get("name").getAsString();
                    }
    	...
    	try {
			...
			for (int i = 0; i < descArray.size(); i++) {
				QoSServerResultDesc desc = new QoSServerResultDesc(descArray.get(i).getAsJsonObject());
