...
//                resultNsec = Arrays.copyOf(resultNsec, results);
    ...
    public RMBTTest(final TestClient client, final TestParameter params, final int threadId,
...
//            setStatus(TestStatus.PACKET_LOSS_AND_JITTER);
    ...
    private boolean upload(final int seconds, final SingleResult result) throws IOException,
        ...
        final Future<Boolean> futureRx = TestClient.getCommonThreadPool().submit(new Callable<Boolean>() {
        ...
        final Future<Void> futureTx = TestClient.getCommonThreadPool().submit(new Callable<Void>() {
